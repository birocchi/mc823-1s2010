...
\subsection{Protocolo de comunicação com UDP}
...
Para isso, foram empacotadas as informações: \textit{opção}, \textit{id} e \textit{nota} num vetor chamado \textit{request}, mesmo para as opções que não requerem id e/ou nota. O envio deste vetor é a considerada pergunta "atômica" e suficiente.
...
Para contornar isso, implementamos uma espécie de \textit{time-out} para o recebimento de respostas pelo cliente:\\
%TODO!!!%
recv não bloqueante + for(vários) { sleep curto }
%TODO!!!%
