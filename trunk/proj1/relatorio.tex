...
% TODO
%%%%%%%%
No fim, a quantidade de linhas de código de todos os arquivos do sistema (*.c e *.h) é: .\\
%%%%%%%%
% \TODO
%%%%%%%%
...
% TODO
%%%%%%%%
Explicação do jeito como foi feita a contagem do tempo (falar sobre o gettimeofday...)
%%%%%%%%
% TODO
%%%%%%%%
...
IP local: ???.???.???.???\\
IP remoto: ???.???.???.???\\
%% TODO: execução do traceroute
%% essa que coloquei é um exemplo; não usar esses valores, pq estou no IC de uma máquina pra outra daqui
\begin{verbatim}
[ra070803@dickens proj1]$ traceroute bocaccio
traceroute to bocaccio (143.106.16.211), 30 hops max, 60 byte packets
 1  bocaccio.lab.ic.unicamp.br (143.106.16.211)  1.372 ms  1.369 ms  1.358 ms
\end{verbatim}
\textbf{Configuração 2: }Um computador no laboratório do IC3 e outro no SIFEEC.\\
\begin{verbatim}
Acessar o IC por ssh, dar um traceroute do IC pro IP da máquina da FEEC e copiar o resultado aqui.
\end{verbatim}
\textit{Observação: }Todos os tempos que serão mostrados nas próximas seções estão em micro-segundos.

\subsection{Tempo de Comunicação}
\subsubsection{Estabelecimento de conexão}
